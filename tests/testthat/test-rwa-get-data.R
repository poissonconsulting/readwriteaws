...
test_that("get 2 image related files", {
  file_list <- rwa_get_data(bucket_name = bucket_name, data_type = "image")
  expect_equal(length(file_list), 2L)
})

test_that("get 2 logger related files", {
  file_list <- rwa_get_data(bucket_name = bucket_name, data_type = "logger")
  expect_equal(length(file_list), 2L)
})

test_that("get 4 pdf related files", {
  file_list <- rwa_get_data(bucket_name = bucket_name, data_type = "pdf")
  expect_equal(length(file_list), 4L)
})

test_that("get 3 punch data related files", {
  file_list <- rwa_get_data(bucket_name = bucket_name, data_type = "punch-data")
  expect_equal(length(file_list), 3L)
})

test_that("get 3 tracks related files", {
  file_list <- rwa_get_data(bucket_name = bucket_name, data_type = "tracks")
  expect_equal(length(file_list), 3L)
})
...
test_that("error when year is not passed as string", {
  expect_error(rwa_get_data(bucket_name = bucket_name, year = 2021),
               regexp = "must be a string")
})

test_that("error when month is not passed as string", {
  expect_error(rwa_get_data(bucket_name = bucket_name, month = 07),
               regexp = "must be a string")
})

test_that("error when month is not passed as string", {
  expect_error(rwa_get_data(bucket_name = bucket_name, day = 30),
               regexp = "must be a string")
})
